// ============================
// 1. الاستيرادات الرئيسية (بدون تكرار)
// ============================
...
اعطِ إجابة عملية وواضحة في فقرة أو فقرتين بالكثير، وإذا ما تقدر تجاوب على شيء فاذكر أن المعلومة غير متوفرة بدقة.`;
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-blue-200 selection:text-blue-900" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      {/* زر تبديل اللغة: يظهر داخل شريط التنقل كنص EN/AR */}
      ...
      {/* شريط التنقل - يسار الشاشة، تصميم شفاف، زر اللغة كنص EN/AR */}
      <nav className={`fixed top-2 md:top-6 left-0 right-0 z-50 flex pointer-events-none ${view === 'home' ? 'pl-4 md:pl-6 pr-32 md:pr-48 lg:pr-64 justify-start' : 'justify-center px-2 md:px-4'}`}>
        <div className="backdrop-blur-sm bg-blue-950/30 border border-white/10 rounded-full px-1 py-1 md:px-2 md:py-2 flex items-center flex-nowrap gap-1 md:gap-2 pointer-events-auto max-w-full overflow-x-auto no-scrollbar shadow-2xl shadow-blue-950/20">
          ...
          <button
            ...
            className="px-3 py-1.5 md:py-2 rounded-full font-bold text-xs text-white/90 hover:text-white border border-white/10 hover:border-white/20 transition-all shrink-0"
            title={t.langName}
          >
            {lang === 'ar' ? 'EN' : 'AR'}
          </button>
        ...
        <>
          ...
          <div className="bg-gradient-to-b from-slate-950 via-blue-950 to-indigo-950 text-white pt-32 md:pt-40 pb-64 md:pb-80 pl-4 md:pl-6 pr-32 md:pr-48 lg:pr-64 relative overflow-hidden rounded-b-2xl md:rounded-b-[5rem] shadow-2xl">
            ...
            <div className="max-w-4xl mr-auto text-left relative z-10">
              ...
              <p className="text-blue-100 text-base md:text-lg mb-12 max-w-2xl mr-auto font-medium leading-relaxed opacity-90 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-100">{t.heroDesc}</p>
              
              <form onSubmit={handleSearch} className="relative max-w-2xl group animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-200">
              ...
              <button
                ...
                className="mt-4 ml-0 max-w-xs flex items-center justify-center gap-2 py-3 px-6 rounded-2xl bg-white/10 hover:bg-white/20 border border-white/20 text-white font-bold transition-all active:scale-95 backdrop-blur-md"
          ...
          <main className="max-w-7xl mr-auto ml-4 md:ml-6 px-4 md:px-6 -mt-16 md:-mt-20 relative z-20">
            ...
            {!results && !isSearching && (
              <>
                {/* 1. وش مقارن؟ (About Us) - تصميم مضغوط وأنيق */}
                <section id="about" className="mb-20 scroll-mt-32 mt-20">
                  <div className="text-left mb-8">
                    <h2 className="text-2xl md:text-3xl font-black text-slate-900 mb-2">{t.howItWorksTitle}</h2>
                    <p className="text-slate-500 font-bold text-sm md:text-base">{t.threeStepsDesc}</p>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                    {[{ icon: MousePointer2, title: t.step1Title, desc: t.step1Desc, color: 'blue', bg: 'bg-blue-500/10', iconBg: 'bg-blue-500', text: 'text-blue-600' }, { icon: Cpu, title: t.step2Title, desc: t.step2Desc, color: 'indigo', bg: 'bg-indigo-500/10', iconBg: 'bg-indigo-500', text: 'text-indigo-600' }, { icon: Rocket, title: t.step3Title, desc: t.step3Desc, color: 'green', bg: 'bg-emerald-500/10', iconBg: 'bg-emerald-500', text: 'text-emerald-600' }].map((item, i) => (
                      <div key={i} className="flex items-start gap-4 p-4 md:p-5 rounded-xl bg-slate-50/80 border border-slate-200/80 hover:border-slate-300 hover:shadow-md transition-all group">
                        <div className={`shrink-0 w-10 h-10 rounded-full ${item.iconBg} flex items-center justify-center text-white shadow-sm group-hover:scale-105 transition-transform`}>
                ...
                {/* 2. ليش تثق فينا؟ (Why Trust Us) - تصميم معكوس ومحاذاة لليمين */}
                <section id="why-trust" className="mb-20 scroll-mt-32">
                  <div className="text-right mb-8">
                    <h2 className="text-2xl md:text-3xl font-black text-slate-900 mb-2">{t.trustTitle}</h2>
                    <div className="inline-flex items-center gap-2 bg-blue-50 text-blue-900 px-4 py-2 rounded-full font-black text-sm">
                      <Activity size={18} className="text-blue-600" />
                      <span>{realSearchCount.toLocaleString()} {t.realSearch}</span>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 ml-0 md:ml-auto md:max-w-3xl">
                    {[
                      { icon: BarChart3, title: t.trust1Title, desc: t.trust1Desc, accent: 'bg-blue-500', card: 'bg-blue-50/90 border-blue-200/60' },
                      { icon: Shield, title: t.trust2Title, desc: t.trust2Desc, accent: 'bg-emerald-500', card: 'bg-emerald-50/90 border-emerald-200/60' },
                      { icon: Heart, title: t.trust3Title, desc: t.trust3Desc, accent: 'bg-violet-500', card: 'bg-violet-50/90 border-violet-200/60' }
                    ].map((item, i) => (
                      <div key={i} className={`flex items-start gap-4 p-4 md:p-5 rounded-xl border ${item.card} hover:shadow-md transition-all group text-right md:flex-row-reverse`}>
                        <div className={`shrink-0 w-10 h-10 rounded-full ${item.accent} flex items-center justify-center text-white shadow-sm group-hover:scale-105 transition-transform`}>
                          <item.icon className="w-5 h-5" />
                        </div>
